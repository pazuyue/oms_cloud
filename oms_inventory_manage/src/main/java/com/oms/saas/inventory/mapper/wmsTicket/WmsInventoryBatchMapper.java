...
public interface WmsInventoryBatchMapper extends BaseMapper<WmsInventoryBatch> {
    ...
    @Insert("<script>" +
            " INSERT INTO wms_inventory_batch(store_code,sku_sn,zp_actual_number,cp_actual_number,zp_available_number,cp_available_number,remark,batch_code,transaction_price,brand_code) VALUES " +
            " (#{wms.storeCode},#{wms.skuSn},#{wms.zpActualNumber},#{wms.cpActualNumber},#{wms.zpAvailableNumber},#{wms.cpAvailableNumber},#{wms.remark},#{wms.batchCode},#{wms.transactionPrice},#{wms.brandCode})" +
