...
public class TestFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        setToken(servletRequest);
        filterChain.doFilter(servletRequest, servletResponse);// 处理请求和响应的分界线
    }

    private void setToken(ServletRequest servletRequest) {
        ...
        if (!StrUtil.isBlank(token)){
           ...
           try {
               ...
               System.out.println("===> chain.doFilter 后执行处理 jsonObject 的相关方法"+jsonObject.toString());
